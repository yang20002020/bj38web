...
import (
	"bj38web_071/web/model"
	"bj38web_071/web/proto/getCaptcha"
	userMico "bj38web_071/web/proto/user"
	"bj38web_071/web/utils"
	"context"
	"encoding/json"
	"fmt"
...
func GetSession(ctx *gin.Context) {
	...
	//func PostLogin(ctx *gin.Context)  函数中已经定义了userName名称
	...
	if userName == nil {
		...
		/*{"data":{"name":"15889**********"},"errmsg":"成功","errno":"0"}*/
		var nameData struct {
...
func GetSmscd(ctx *gin.Context) {
	...
	//http://192.168.1.112:8080/api/v1.0/smscode/1588******?text=enhe&id=56b89c6b-d62d-45c2-b84d-e9d80d8f187c
...
func PostLogin(ctx *gin.Context) {
	...
	ctx.Bind(&loginData)
	fmt.Println("获取登录数据：")
	fmt.Println("mobile；", loginData.Mobile, "password:", loginData.PassWord)
	resp := make(map[string]interface{})
	//获取数据库数据 查询登录输入的数据是否和数据库的数据匹配
