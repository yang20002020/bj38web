...
import (
	"bj38web_058/web/model"
	"bj38web_058/web/proto/getCaptcha"
	"bj38web_058/web/utils"
	"context"
	"encoding/json"
	"fmt"
...
func GetSmscd(ctx *gin.Context) {
	...
	//http://192.168.***.112:8080/api/v1.0/smscode/158****?text=enhe&id=56b89c6b-d62d-45c2-b84d-e9d80d8f187c
	//拆分GET请求中的URL ===格式： 资源路径？/k=v & k=v & k =v
	//参考gin框架 中文文档
	imgCode := ctx.Query("text") //query 查询
	uuid := ctx.Query("id")
	fmt.Println("---OUT----:", phone, imgCode, uuid)
	fmt.Println("phone：", phone)
	fmt.Println("imgCode:", imgCode)
	fmt.Println("uuid:", uuid)
	//校验图片验证码 是否正确
	...
	if result {
		...
		fmt.Println("校验成功！！！")
		//发送短信
		err := _main(tea.StringSlice(os.Args[1:]), phone, resp) //第一个参数是默认参数
		if err != nil {
			panic(err)
		}
	} else {
...
func _main(args []*string, phone string, resp map[string]string) (_err error) {
	...
	//TemplateParam: tea.String("{\"code\":\"666777\"}"),
	subStr := "{\"code\"" + ":" + "\"" + smsCode + "\"" + "}"
	//"{\"code\":\"666777\"}"
	fmt.Println(subStr)
	sendSmsRequest := &dysmsapi20170525.SendSmsRequest{
		SignName:      tea.String("阿里云短信测试"),
		TemplateCode:  tea.String("SMS_154950***"),
