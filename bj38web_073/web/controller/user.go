...
import (
	...
	"bj38web_073/web/proto/getCaptcha"
	userMico "bj38web_073/web/proto/user"
	"bj38web_073/web/utils"
	"context"
	"encoding/json"
	"fmt"
...
func GetUserInfo(ctx *gin.Context) {
	...
	resp["errno"] = utils.RECODE_OK
	resp["errmsg"] = utils.RecodeText(utils.RECODE_OK)
	//{"data":{"avatar_url":"","id_card":"","mobile":"1588***","name":"158****",
...
func PutUerInfo(ctx *gin.Context) {
	...
	s := sessions.Default(ctx)
	userName := s.Get("userName")
	//2.获取新用户名 ---处理request payload 类型数据 bind
	var nameData struct {
		Name string `json:"name"`
	}
	ctx.Bind(&nameData) //此时 新名字已经存储在namedata中
	//3. 更新用户名
	//3-1.更新mysql数据库中的name
	...
	resp["errmsg"] = utils.RecodeText(utils.RECODE_OK)
	//参考文档
	resp["data"] = nameData //  nameData 是结构体
